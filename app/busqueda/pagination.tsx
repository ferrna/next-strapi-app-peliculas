...
export default async function Pagination({
  ...
  const { data, total } = await getData(Number(page), start, param || '')
...
async function getData(page: number, start: number, param: string) {
  const total = await fetch(`${URL_API}/api/peliculas?filters[titulo][$contains]=${param}&populate=*`, {
    next: { revalidate: 30 },
  })
    .then(async (res) => await res.json())
    .then((res) => res.meta.pagination.total)
  const res = await fetch(
    `${URL_API}/api/peliculas?sort=createdAt:DESC&pagination[start]=${start}&pagination[limit]=3&filters[titulo][$contains]=${param}&populate=*`,
    {
      next: { revalidate: 30 },
    }
  )
